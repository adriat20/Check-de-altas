...
if resp == 'S':
    ...
    for col in df_tram["COLABORADOR"].dropna().unique():
        ...
        # -------------- LEYENDA --------------------------------------------------------
        leyenda = [
            ('🟩 ALTA',       "Firma entre d_ini y d_fin, sin caída",      'alta'),
            ('🟥 BAJA',       "Caída entre d_ini y d_fin",                  'baja'),
            ('🟨 INCIDENCIA', "Firma en rango, sin alta ni caída",         'inci'),
            ('🟦 SECUNDARIO',"Firma < d_ini y caída entre d_ini y hoy",    'sec'),
        ]
        legend_start = start_blank + 4
        border_top   = Border(top=Side(style="medium"))
        border_bot   = Border(bottom=Side(style="medium"))
        for off, (lbl, desc, key) in enumerate(leyenda):
            row = legend_start + off
            # Fusiona celdas 1:4
            ws.merge_cells(start_row=row, start_column=1,
                           end_row=row,   end_column=4)
            cell = ws.cell(row=row, column=1, value=f"{lbl}: {desc}")
            cell.fill      = fills[key]
            cell.font      = Font(italic=True)
            cell.alignment = Alignment(horizontal="left", vertical="center")
            # Establece fila un poco más alta
            ws.row_dimensions[row].height = 20

            # Aplica borde grueso solo en la primera y última línea
            if off == 0:
                cell.border = border_top
            elif off == len(leyenda) - 1:
                cell.border = border_bot


    wb.save(out)
    print(f"✅ Agregadas {df_tram['COLABORADOR'].nunique()} hojas de TRAMITACIÓN al archivo {out}")
